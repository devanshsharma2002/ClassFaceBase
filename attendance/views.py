...
from .models import AttendanceDispute, AttendancePhoto, AttendanceRecord, AttendanceSession
...
def get_students_for_session(session):
    enrolled_students = StudentProfile.objects.filter(
        ...
        subject_enrollments__class_section=session.class_section,
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if students:
                ...
                messages.warning(
                    request,
                    "Session created, but no matching students were found for this subject/section/year.",
                )
...
def create_demo_session(request):
    ...
    assignment = get_object_or_404(
        ProfessorSubjectAssignment.objects.select_related(
            "subject__section",
...
def session_review_view(request, session_id):
    ...
    if request.method == "POST":
        ...
        for record in records:
            record.status = request.POST.get(
                f"status_{record.id}",
        ...
        if "finalize" in request.POST:
            ...
            if success:
                ...
                messages.warning(
                    request,
                    f"Session finalized, but email failed: {error}",
                )
